...
<head>
    ...
    <!-- KICKSTART -->
    <title>Cliente SGF - By Igor Menin (Secretaria Municipal da Saúde)</title>
</head>
...
<form action="./validaCliente.php" method="post">
    <div class="container">
        ...
        <div class="contents">
            <h3><b>SGF - Sistema de Gerenciamento de Filas<br>Secretaria Municipal da Saúde - PMJS</b></h3>

            <div class="h3 tooltip"><b>Usuário: </b><?= ' ' . $_SESSION['des_usuario'] ?>
                <span class="tooltiptext"><?= $_SESSION['des_nome'] ?></span>
            </div>
            <div class="h3"><b>Estação: </b><?= $_SESSION['hostNameClient']; ?> (
                <b><i><?= $_SESSION['localAtual'] ?></i></b> )
            </div>
        </div>
        <div class="senhas">
            <h6>SENHA ATUAL</h6>
        ...
        <div class="divButtons">
            ...
            if ($btnRechamarVisible == '') {
                $bloqueiaRechamar = '';
                $retVal = validaBloquearRechamar();
                if (strtoupper($retVal) == 'NO') {
//                    $bloqueiaRechamar = ' disabled';
                    echo '<button id="btnRechamar" type="submit" name="btnRechamar" class="col_12 fa fa-refresh" tabindex="1" ' . $bloqueiaRechamar . '> Re-Chamar</button>';
                }
            }
